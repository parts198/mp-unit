...
def product_info_list_v3(client: OzonClient, offer_ids: List[str]) -> List[Dict[str, Any]]:
    # /v3/product/info/list: можно передать offer_id[] до 1000 значений суммарно
    payload = {"offer_id": offer_ids}
    resp = client.post("/v3/product/info/list", payload)
    # частые варианты: {"result":{"items":[...]}} или {"items":[...]}
    items = (resp.get("result") or {}).get("items") if isinstance(resp.get("result"), dict) else None
    if items is None:
...
def build_offer_to_sku_map(stores: List[Dict[str, str]], store_sel: Any, offer_ids: List[str], min_interval: float) -> Dict[str, int]:
    stores_all = load_stores()
    selected = normalize_store_selection(store_sel, stores_all)
    # достаточно одного магазина: sku одинаковы в рамках seller
    st = selected[0]
...
# -------------------- OZON stock via /v1/analytics/stocks (by sku) --------------------

def analytics_stocks(client: OzonClient, skus: List[int]) -> List[Dict[str, Any]]:
    # /v1/analytics/stocks: требуется skus[] <= 100
    resp = client.post("/v1/analytics/stocks", {"skus": [int(x) for x in skus]})
    items = resp.get("items")
    if not isinstance(items, list):
        # иногда {result:{items:...}}
        items = (resp.get("result") or {}).get("items") if isinstance(resp.get("result"), dict) else []
    if not isinstance(items, list):
        return []
    return [x for x in items if isinstance(x, dict)]
...
def ozon_postings_list(client: OzonClient, schema: str, since_iso: str, to_iso: str, limit: int = 1000) -> List[Dict[str, Any]]:
    ...
    for _ in range(0, 400):
        ...
        items = (resp.get("result") or {}).get("postings") or (resp.get("result") or {}).get("items") or resp.get("postings") or resp.get("items") or []
        if isinstance(resp.get("result"), list) and not items:
...
def build_recommendations(vel: Dict[Tuple[str, str], Dict[str, Any]], cover_days: int, my_stock: Dict[str, int], ozon_stock: Dict[Tuple[str, str], int]) -> List[Dict[str, Any]]:
    by_offer = defaultdict(list)
    for (cl, offer), v in vel.items():
        avg = float(v.get("avg_daily_sold", 0.0) or 0.0)
        target = avg * float(cover_days)
        cur_ozon = ozon_stock.get((cl, offer))
        cur = float(cur_ozon) if cur_ozon is not None else float(v.get("last_stock_est", 0) or 0)
        rec = max(0.0, target - cur)
        by_offer[offer].append((cl, v, rec, cur_ozon))
    ...
    for offer, lst in by_offer.items():
        ...
        for cl, v, rec, cur_ozon in lst:
            ...
            rows.append({
                ...
                "target": round(float(v.get("avg_daily_sold", 0.0)) * cover_days, 2),
                "current_ozon": int(cur_ozon) if cur_ozon is not None else None,
                "current_est": int(v.get("last_stock_est", 0) or 0),
    ...
    rows.sort(key=lambda x: (x.get("recommended_capped", 0), x.get("recommended", 0)), reverse=True)
    return rows
...
def api_analyze(payload: Dict[str, Any]):
    ...
    demand_outflows = build_outflows_from_postings(postings_obj, mode=mode)
    demand = defaultdict(int)
    for ev in demand_outflows:
        if ev.get("bucket") != "Доставлено":
            continue
        demand[(ev["cluster"], ev["offer_id"])] += int(ev["qty"])
    demand_top = sorted(
        [{"cluster": k[0], "offer_id": k[1], "delivered_qty": v} for k, v in demand.items()],
    ...
    recs = build_recommendations(vel, cover_days=cover_days, my_stock=my_stock, ozon_stock=oz_stock)
    ...
    return {
        "ok": True,
        "meta": {"date_from": date_from, "date_to": date_to, "mode": mode, "cover_days": cover_days},
...
def main():
    ...
    args = ap.parse_args()
    uvicorn.run("ozon_backend_v3:app", host=args.host, port=args.port, reload=False)
